...
export default function CrackPage() {
  ...
  useEffect(() => {
    async function fetchContent() {
      try {
        const response = await fetch(`/cracks/raw/${slug}.lua`)
        if (response.ok) {
  ...
  const loadstringCode = `loadstring(game:HttpGet("${window.location.origin}/cracks/raw/${slug}.lua"))()`
          ...
          <Card className="overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b">
              <div>
                ...
                <p className="text-sm text-muted-foreground">View the raw Lua code for this crack</p>
              </div>
              <Button
                variant="outline"
                size="sm"
